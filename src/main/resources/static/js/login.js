$(function () {
        ...
        if (tab == 'account_number') {
            $(".log-btn").click(function () {
                ...
                if (checkAccount(inp) && checkPass(pass)) {
                    var ldata = {email: inp, password: pass};
                    if (!$('.code').hasClass('hide')) {
                    ...
                    if (!isEmail(inp)) {
                        $('#email_login_error').removeClass('hide');
                        $('#email_login_error_mes').text('请输入正确的邮箱!');
                        return;
                    }
                    $.ajax({
                        url: '/security/login',
                        ...
                        data: ldata,
                        success: function (data) {
                            if (data == "USER_LOGIN_SUCCESS") {
                                window.location.href = '/disk/all';
                            } else if (data == "USER_LOGIN_STATUS") {
                                $('#email_login_error').removeClass('hide');
                                $('#email_login_error_mes').text('用户已经登录!');
                            } else if (data == "USER_LOGIN_CODE_ERROR") {
                                $('#email_error_code').removeClass('hide');
                                $('#code_image').click();
                            } else if (data == "USER_LOGIN_USER_ERROR") {
                                $('#email_login_error').removeClass('hide');
                            } else if (data == "USER_LOGIN_PASS_ERROR") {
                                $('#email_error_pass').removeClass('hide');
                            }
                        },
                        error: function () {
    ...
    $('#code_image').click(function () {
        var addr = new Date().getSeconds();
        $(this).attr('src', '/security/page/verification_code?time=' + addr)
    });
});
